...
4. **It tells you what's missing, not just what's good.** The gaps module
   ...
   `likely_required` — so a proposal reads like a punch list a real
   developer could hand to an engineer, not a score.
...
6. **It covers all fifty states without faking coverage it doesn't have.**
   ...
   that state's own GIS endpoints. Right now that's true for all fifty —
...
- **Frontend:** Next.js 14 (App Router) + React 18 + TypeScript. MapLibre GL
  ...
  scenarios and their results, comparison selection — and every panel
