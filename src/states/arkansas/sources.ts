...
/**
 * Canonical source metadata for Arkansas. Mirrors src/states/oklahoma/sources.ts:
 * no state-specific GIS dataset was found for water-rights/withdrawal permits
 * (Arkansas Natural Resources Division regulates them, but publishes no public
 * point-level GIS layer) or road functional classification, so this state's
 * bundle in src/lib/gis/stateGis.ts marks those UNKNOWN rather than guessing.
 * Every layer below is nationwide, reused from NATIONAL_SOURCES — see Step 4
 * in the integration brief this follows ("do not duplicate national data").
 */
export const AR_SOURCES = {
  curatedDataCenters: {
    id: "curated-ar-datacenters",
    name: "Curated Known/Announced Data Center Campuses (Arkansas)",
    url: "",
    license: "Compiled from public reporting; not an authoritative registry",
    refreshFrequency: "Manually maintained",
    methodology: "Hand-curated illustrative list of publicly reported Arkansas data center campuses/projects. NOT exhaustive and NOT independently verified against parcel/operator records.",
  },
