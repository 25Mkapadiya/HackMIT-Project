...
/**
 * Full layer registry for California — mirrors src/states/virginia/layers.ts.
 * No "water-diversions" or "state-highways" layers — see sources.ts.
 */
export const CA_LAYERS: LayerDefinition[] = [
  // ---------------------------------------------------------------- POWER
  {
    ...
    category: "power",
    geometryType: "line",
    source: CA_SOURCES.hifldTransmission,
    confidence: "fact",
    description:
      "HIFLD transmission line geometry and voltage class (nationwide extract, static - see source note). California's grid is largely CAISO-operated with PG&E, SCE and SDG&E as the major transmission owners, plus municipal systems (LADWP, SMUD). Line proximity does not establish available MW or substation headroom.",
    defaultVisible: true,
  ...
  },
  {
    ...
    source: CA_SOURCES.cecUtilityAreas,
    confidence: "proxy",
    description:
      "California Energy Commission electric load-serving-entity boundaries (IOUs and publicly owned utilities). Per the CEC, boundaries are approximate and compiled from georeferenced maps and DHS data - contact the load-serving entity for authoritative territory.",
  ...
  },
  {
    ...
    category: "power",
    geometryType: "point",
    source: CA_SOURCES.hifldSubstations,
    confidence: "fact",
    description:
  ...
  // ---------------------------------------------------------------- WATER
  {
    ...
    category: "water",
    geometryType: "line",
    source: CA_SOURCES.usgsNhdFlowline,
    confidence: "fact",
    description: "USGS National Hydrography Dataset flowlines (small-scale), including the Sacramento, San Joaquin and Colorado River systems. Proximity does not imply a water right or supply availability.",
    defaultVisible: true,
  ...
  },
  {
    ...
    category: "water",
    geometryType: "polygon",
    source: CA_SOURCES.usgsNhdWaterbody,
    confidence: "fact",
  ...
  },
  {
    ...
    category: "water",
    geometryType: "point",
    source: CA_SOURCES.usgsNwisGauges,
    confidence: "fact",
  ...
  },
  {
    ...
    category: "water",
    geometryType: "polygon",
    source: CA_SOURCES.usDroughtMonitor,
    confidence: "fact",
  ...
  // --------------------------------------------------------- CONNECTIVITY
  {
    ...
    geometryType: "point",
    source: CA_SOURCES.peeringDb,
    confidence: "fact",
    description:
      "PeeringDB-listed carrier hotels and colocation facilities - a proxy for interconnection density (Silicon Valley, Los Angeles). Not proof of long-haul fiber or dark-fiber availability.",
    defaultVisible: true,
  ...
  // ----------------------------------------------------------- ENVIRONMENT
  {
    ...
    geometryType: "polygon",
    source: CA_SOURCES.femaNfhl,
    confidence: "fact",
    description: "Effective FEMA National Flood Hazard Layer zones, where mapped. Sacramento Valley, San Joaquin Valley, the Delta and coastal zones are priority areas to check.",
    defaultVisible: false,
  ...
  },
  {
    ...
    category: "environment",
    geometryType: "raster",
    source: CA_SOURCES.cartoForestCover,
    confidence: "proxy",
    description:
  ...
  },
  {
    ...
    category: "environment",
    geometryType: "raster",
    source: CA_SOURCES.usgsShadedRelief,
    confidence: "fact",
    description:
      "USGS The National Map cached shaded relief, derived from 3DEP at large and medium scales - terrain context only. Seismic (fault) and wildfire exposure require separate review.",
    defaultVisible: false,
    color: "#8d8a82",
  ...
  },
  {
    ...
    category: "community",
    geometryType: "polygon",
    source: CA_SOURCES.censusCountyDensity,
  ...
  // ----------------------------------------------------- EXISTING INFRA
  {
    ...
    geometryType: "point",
    source: CA_SOURCES.eia,
    confidence: "unknown",
    description:
      "Nearby generating facilities from EIA - California's solar, gas, hydro, geothermal, wind and nuclear fleet. Display as NEARBY GENERATION only; nameplate capacity is not available power. Requires a server-side EIA_API_KEY - shown as unavailable until configured.",
    defaultVisible: true,
  ...
  },
  {
    ...
    category: "existing_infrastructure",
    geometryType: "point",
    source: CA_SOURCES.curatedDataCenters,
    confidence: "proxy",
    description: "Hand-curated, illustrative list of publicly reported California data center markets (Silicon Valley/Santa Clara, Sacramento, Los Angeles, Inland Empire). Area coordinates only, not exhaustive; MW is UNKNOWN unless operator-stated.",
    defaultVisible: true,
...
export function getVaLayer(id: string): LayerDefinition | undefined {
  return CA_LAYERS.find((l) => l.id === id);
}
