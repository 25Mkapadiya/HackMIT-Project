...
import { HI_SOURCES } from "./sources";

/**
 * Full layer registry for Hawaii — mirrors src/states/virginia/layers.ts.
 * No "water-diversions" or "state-highways" layers — see sources.ts.
 */
export const HI_LAYERS: LayerDefinition[] = [
  // ---------------------------------------------------------------- POWER
  {
    ...
    category: "power",
    geometryType: "line",
    source: HI_SOURCES.hifldTransmission,
    confidence: "fact",
    description:
      "HIFLD transmission line geometry and voltage class (nationwide extract, static - see source note). Each Hawaiian island runs its own isolated grid (HECO on Oahu, HELCO on Hawaii Island, MECO on Maui/Lanai/Molokai, KIUC on Kauai), so a line on one island says nothing about another. Line proximity does not establish available MW or substation headroom.",
    defaultVisible: true,
  ...
  },
  {
    ...
    source: HI_SOURCES.hifldUtilityTerritories,
    confidence: "proxy",
    description:
      "HIFLD electric retail service territory boundaries (static 2025-08-21 snapshot). Informational - not an official service determination; Hawaii's island utilities (HECO, HELCO, MECO, KIUC) are separately regulated by the Hawaii PUC; confirm the serving utility directly.",
  ...
  },
  {
    ...
    category: "power",
    geometryType: "point",
    source: HI_SOURCES.hifldSubstations,
    confidence: "fact",
    description:
  ...
  // ---------------------------------------------------------------- WATER
  {
    ...
    category: "water",
    geometryType: "line",
    source: HI_SOURCES.usgsNhdFlowline,
    confidence: "fact",
    description: "USGS National Hydrography Dataset flowlines (small-scale) for Hawaii's island stream systems. Proximity does not imply water-use permit availability (CWRM administers water-use permits).",
    defaultVisible: true,
  ...
  },
  {
    ...
    category: "water",
    geometryType: "polygon",
    source: HI_SOURCES.usgsNhdWaterbody,
    confidence: "fact",
  ...
  },
  {
    ...
    category: "water",
    geometryType: "point",
    source: HI_SOURCES.usgsNwisGauges,
    confidence: "fact",
  ...
  },
  {
    ...
    category: "water",
    geometryType: "polygon",
    source: HI_SOURCES.usDroughtMonitor,
    confidence: "fact",
  ...
  // --------------------------------------------------------- CONNECTIVITY
  {
    ...
    geometryType: "point",
    source: HI_SOURCES.peeringDb,
    confidence: "fact",
    description:
      "PeeringDB-listed carrier hotels and colocation facilities - a proxy for interconnection density. Hawaii's trans-Pacific submarine-cable landings are not represented; not proof of long-haul or subsea capacity.",
    defaultVisible: true,
  ...
  // ----------------------------------------------------------- ENVIRONMENT
  {
    ...
    geometryType: "polygon",
    source: HI_SOURCES.femaNfhl,
    confidence: "fact",
    description: "Effective FEMA National Flood Hazard Layer zones, where mapped. Coastal, tsunami-adjacent, stream-valley and flash-flood corridors are priority checks.",
    defaultVisible: false,
  ...
  },
  {
    ...
    category: "environment",
    geometryType: "raster",
    source: HI_SOURCES.cartoForestCover,
    confidence: "proxy",
    description:
  ...
  },
  {
    ...
    category: "environment",
    geometryType: "raster",
    source: HI_SOURCES.usgsShadedRelief,
    confidence: "fact",
    description:
      "USGS The National Map cached shaded relief - terrain context only. Volcanic (Hawaii Island), seismic, tsunami and salt-air corrosion exposure require separate review.",
    defaultVisible: false,
    color: "#8d8a82",
  ...
  },
  {
    ...
    category: "community",
    geometryType: "polygon",
    source: HI_SOURCES.censusCountyDensity,
  ...
  // ----------------------------------------------------- EXISTING INFRA
  {
    ...
    geometryType: "point",
    source: HI_SOURCES.eia,
    confidence: "unknown",
    description:
      "Nearby generating facilities from EIA - Hawaii's oil, solar, wind, geothermal, hydro and battery fleet. Display as NEARBY GENERATION only; because each island grid is isolated, generation on another island is not relevant to a site. Requires a server-side EIA_API_KEY.",
    defaultVisible: true,
  ...
  },
  {
    ...
    category: "existing_infrastructure",
    geometryType: "point",
    source: HI_SOURCES.curatedDataCenters,
    confidence: "proxy",
    description: "No facility points are shown: the research names Honolulu and the Oahu technology corridor as markets but provides no verified facility coordinates.",
    defaultVisible: true,
...
export function getVaLayer(id: string): LayerDefinition | undefined {
  return HI_LAYERS.find((l) => l.id === id);
}
