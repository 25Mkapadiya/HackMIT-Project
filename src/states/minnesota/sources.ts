...
/**
 * Canonical source metadata for every Minnesota-specific dataset the platform
 * uses. National datasets (EIA, FEMA, Census, USGS, PeeringDB) live in
 * src/lib/gis/nationalSources.ts instead and are merged in by analysisContext.ts.
 *
 * URLs were live-verified (actual test queries run, not guessed from naming
 * patterns) on 2026-09-19. Two items the MN research doc named — an
 * MnGeo-official transmission-line layer and a DNR water-appropriation-permit
 * GIS service — turned out not to exist as public, queryable endpoints
 * (MnGeo's own utilities page says the transmission dataset was withdrawn;
 * DNR's water-permit system, MPARS, is a login-gated web app, not a REST
 * service). Rather than fabricate URLs for those, transmission falls back to
 * a national HIFLD-schema mirror that covers MN, and water rights is left
 * unintegrated (UNKNOWN) — see minnesota/analysisContext.ts.
 */
export const MN_SOURCES = {
  transmissionNational: {
    id: "hifld-mirror-transmission",
    name: "Electric Power Transmission Lines (national HIFLD-schema mirror)",
    ...
    methodology: "Queried by bounding box; voltage from the VOLTAGE attribute (kV).",
  ...
  substationsNational: {
    ...
    license: "Public (HIFLD-derived, hosted mirror)",
    refreshFrequency: "Live ArcGIS FeatureServer",
    methodology: "Queried by bounding box, filtered where STATE='MN'.",
  } satisfies SourceMeta,
  utilityServiceAreas: {
    id: "mn-eusa",
    ...
    refreshFrequency: "Live ArcGIS FeatureServer",
    methodology: "Point-in-polygon lookup against MnGeo/PUC-sourced utility service-area boundaries.",
  } satisfies SourceMeta,
  ...
  curatedDataCenters: {
    ...
    methodology: "No verified, geolocated Minnesota data center facilities have been catalogued yet — this is deliberately empty rather than guessed.",
  } satisfies SourceMeta,
