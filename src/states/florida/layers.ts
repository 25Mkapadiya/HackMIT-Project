...
import { FL_SOURCES } from "./sources";

/**
 * Full layer registry for Florida — mirrors src/states/kentucky/layers.ts.
 * No "water-diversions" or "state-highways" layers — see sources.ts.
 */
export const FL_LAYERS: LayerDefinition[] = [
  // ---------------------------------------------------------------- POWER
  {
    ...
    category: "power",
    geometryType: "line",
    source: FL_SOURCES.hifldTransmission,
    confidence: "fact",
    description:
      "HIFLD transmission line geometry and voltage class (nationwide extract, static — see source note). Florida is served by Florida Power & Light (FPL), Duke Energy Florida, Tampa Electric (TECO), and others, largely as a standalone grid rather than a member of a traditional RTO.",
    defaultVisible: true,
  ...
  },
  {
    ...
    source: FL_SOURCES.hifldUtilityTerritories,
    confidence: "proxy",
    description:
      "HIFLD electric retail service territory boundaries (static 2025-08-21 snapshot). Informational — not an official service determination; Florida's own PSC also maintains territory maps.",
  ...
  },
  {
    ...
    category: "power",
    geometryType: "point",
    source: FL_SOURCES.hifldSubstations,
    confidence: "fact",
    description:
  ...
  // ---------------------------------------------------------------- WATER
  {
    ...
    category: "water",
    geometryType: "line",
    source: FL_SOURCES.usgsNhdFlowline,
    confidence: "fact",
    description: "USGS National Hydrography Dataset flowlines (small-scale), including the St. Johns, Suwannee, Kissimmee, and Apalachicola River systems.",
    defaultVisible: true,
  ...
  },
  {
    ...
    category: "water",
    geometryType: "polygon",
    source: FL_SOURCES.usgsNhdWaterbody,
    confidence: "fact",
    description: "USGS National Hydrography Dataset waterbody polygons (small-scale), including Lake Okeechobee and the Everglades' extensive wetland/waterbody mosaic.",
    defaultVisible: true,
  ...
  },
  {
    ...
    category: "water",
    geometryType: "point",
    source: FL_SOURCES.usgsNwisGauges,
    confidence: "fact",
  ...
  },
  {
    ...
    category: "water",
    geometryType: "polygon",
    source: FL_SOURCES.usDroughtMonitor,
    confidence: "fact",
  ...
  // --------------------------------------------------------- CONNECTIVITY
  {
    ...
    geometryType: "point",
    source: FL_SOURCES.peeringDb,
    confidence: "fact",
    description:
      "PeeringDB-listed carrier hotels and colocation facilities — a proxy for interconnection density. Miami is a major subsea-cable landing hub for Latin American connectivity, so this list is illustrative, not exhaustive.",
    defaultVisible: true,
  ...
  // ----------------------------------------------------------- ENVIRONMENT
  {
    ...
    geometryType: "polygon",
    source: FL_SOURCES.femaNfhl,
    confidence: "fact",
    description: "Effective FEMA National Flood Hazard Layer zones, where mapped. Flood, hurricane, and storm-surge risk should be a primary scoring factor for Florida site selection — check South Florida, coastal counties, and low-elevation inland floodplains closely.",
    defaultVisible: false,
  ...
  },
  {
    ...
    category: "environment",
    geometryType: "raster",
    source: FL_SOURCES.cartoForestCover,
    confidence: "proxy",
    description:
      "Visual forest/wood land-cover polygons from the CARTO basemap's OpenStreetMap-derived landcover layer (e.g. Ocala, Apalachicola, and Osceola National Forests). Optimized for clear map reading; not an authoritative forestry inventory.",
    defaultVisible: false,
    color: "#4f7f50",
  ...
  },
  {
    ...
    category: "environment",
    geometryType: "raster",
    source: FL_SOURCES.usgsShadedRelief,
    confidence: "fact",
    description:
      "USGS The National Map cached shaded relief, derived from 3DEP at large and medium scales. Florida's low, flat elevation profile makes flood-zone data a far more useful risk signal here than relief.",
    defaultVisible: false,
    color: "#8d8a82",
  ...
  },
  {
    ...
    category: "community",
    geometryType: "polygon",
    source: FL_SOURCES.censusCountyDensity,
  ...
  // ----------------------------------------------------- EXISTING INFRA
  {
    ...
    geometryType: "point",
    source: FL_SOURCES.eia,
    confidence: "unknown",
    description:
      "Nearby generating facilities from EIA. Turkey Point and St. Lucie nuclear plants plus an extensive natural-gas fleet anchor Florida's generation mix. Requires a server-side EIA_API_KEY — shown as unavailable until configured.",
    defaultVisible: true,
  ...
  },
  {
    ...
    category: "existing_infrastructure",
    geometryType: "point",
    source: FL_SOURCES.curatedDataCenters,
    confidence: "proxy",
    description: "Hand-curated, illustrative list of publicly reported Florida data center campuses/projects. Not exhaustive.",
    defaultVisible: true,
...
export function getFlLayer(id: string): LayerDefinition | undefined {
  return FL_LAYERS.find((l) => l.id === id);
}
