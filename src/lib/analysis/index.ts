 ...
 *
 *   - POWER, FIBER, WATER, and LAND have no dependency on each other, so all
 *     four start immediately, in the background, together.
 ...
 *     NOT wait for FIBER/WATER/LAND to finish too.
 *   - NOISE is synchronous (no network call) and only needs POWER, so it's
 *     computed last, for free.
 *   - FIBER/WATER/LAND are awaited last, by which point they've likely
 ...
 * (e.g. efficiency's and water's own drought/water-rights lookups) costs at
 * most one extra duplicate request per run — see the comment on
 * computeWaterStressContext in water.ts. Deliberately NOT deduped via the
...
export async function runScenarioAnalysis(scenario: ScenarioConfig): Promise<ScenarioAnalysis> {
  ...
  const [fiber, water, land] = await Promise.all([fiberPromise, waterPromise, landPromise]);
