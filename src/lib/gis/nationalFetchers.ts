...
// ----------------------------------------------------- EXISTING INFRA (Supabase, any state)
...
/**
 * Reads existing_data_centers from Supabase filtered by state_code, falling
 * back to a state's own hand-curated static list if Supabase is unavailable
 * or has no rows yet for that state. Shared by every state's layerFetchers
 * module rather than each maintaining its own copy of this logic.
 */
export function makeExistingDataCentersFetcher(
  stateAbbrev: string,
  ...
  return async (bbox: Bbox): Promise<FeatureCollection<Point, DataCenterProps>> => {
    ...
    if (supabase) {
      ...
      // Only trust Supabase once it actually has rows for this state — an empty
      // result (not yet seeded) falls through to the static list below rather
      // than silently discarding a state's hand-curated data centers.
      ...
      if (error) console.error(`[fetchExistingDataCenters:${stateAbbrev}] Supabase query failed:`, error);
    }
