...
export function scoreFromFactors(
  ...
  if (stateRule) {
    const countyRule = stateRule.counties?.[bareCountyName(factors.name)];
    if (countyRule) {
      if (countyRule.status === "blocked") {
        ...
        notes.push(`${countyRule.reason} Score capped pending local reopening.`);
      }
    }

    if (factors.statePostal === "TX") {
