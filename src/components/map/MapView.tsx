...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    function materializeLayer(state: { id: string }, layer: LayerDefinition, data: FeatureCollection) {
      ...
      // Population density loads as a toggle-on layer, often after the
      // transmission-lines-high/low layers are already on the map (both
      // default-visible in most states) — without this, its fill would get
      // added on top and visually bury the lines it's meant to give context
      // to. Anything added later goes on top by default, so insert it before
      // whichever transmission-line tier is already present first.
      const insertBeforeId =
        layer.id === "population-density"
          ? map.getStyle().layers?.find(
              (l) => l.id.endsWith("-transmission-lines-high-line") || l.id.endsWith("-transmission-lines-low-line")
            )?.id
          : undefined;
      for (const spec of buildLayerSpecs(layer, sourceId, state.id)) {
